...
class GoogleOAuth2Service:
    ...
    def _init_essential_sheets(self, sheets_service, spreadsheet_id):
        ...
        try:
            ...
            registry = google_manager._load_sheet_schema()
            schemas = registry.get("schemas", {})
        except Exception as e:
            logger.error(f"Failed to load schemas in _init_essential_sheets: {e}")
            schemas = {}

        essential_sheets = {
            "ใบเสร็จ/ใบกำกับภาษี": schemas.get("ใบเสร็จ/ใบกำกับภาษี", {}).get("headers", []),
            "สลิปโอนเงิน": schemas.get("สลิปโอนเงิน", {}).get("headers", []),
            "ใบหัก ณ ที่จ่าย": schemas.get("ใบหัก ณ ที่จ่าย", {}).get("headers", []),
            "บัตรประชาชน": schemas.get("บัตรประชาชน", {}).get("headers", []),
            "สเตตเมนต์": schemas.get("สเตตเมนต์", {}).get("headers", []),
            "สรุปกระทบยอด": ["วันที่ประมวลผล", "สถานะ", "วันที่เอกสาร", "ยอดเงิน", "คู่ค้า/ร้านค้า", "ลิงก์สลิป", "ลิงก์ใบกำกับ", "หมายเหตุ"],
            "ใบเสนอราคา": schemas.get("ใบเสนอราคา", {}).get("headers", []),
            "peak": schemas.get("peak", {}).get("headers", [])
        }

        # Safe fallbacks if schemas load empty
        if not essential_sheets["ใบเสร็จ/ใบกำกับภาษี"]:
            essential_sheets["ใบเสร็จ/ใบกำกับภาษี"] = [
        ...
        if not essential_sheets["สลิปโอนเงิน"]:
            essential_sheets["สลิปโอนเงิน"] = [
